...
### 출력
- [X] 인사말을 출력한다. (안녕하세요! 우테코 식당 12월 이벤트 플래너입니다.)
- [X] 이벤트 내용 시작 문장을 출력한다. (12월 3일에 우테코 식당에서 받을 이벤트 혜택 미리 보기!)
- [X] 주문 메뉴를 출력한다.
- [X] 할인 전 총주문 금액을 출력한다.
- [X] 증정 메뉴를 출력한다.
...
### cristmasDdayEvent
- [X] 날짜에 따른 할인 금액을 가진다.
- [X] 26일 이후는 이벤트를 적용하지 않는다.

### weekdayEvent
- [X] 디저트 메뉴 1개당 2,023원을 할인한다.
- [X] 일 ~ 목요일이 아닌 경우 이벤트를 적용하지 않는다.

### weekEndEvent
- [X] 메인 메뉴 1개당 2,023원을 할인한다.
- [X] 금 ~ 토요일이 아닌 경우 이벤트를 적용하지 않는다.

### specialEvent
- [X] 별이 있는 경우 1,000원을 할인한다.
- [X] 별이 없는 경우 이벤트를 적용하지 않는다.

### presentEvent
- [X] 할인 전 총주문 금액이 12만원 이상인 경우 샴페인을 1개 증정한다.
